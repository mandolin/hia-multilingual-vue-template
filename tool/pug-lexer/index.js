...
Lexer.prototype = {
  ...
  className: function() {
    var tok = this.scan(/^\.([_a-z0-9\-\u3400-\u9FFF\uF900-\uFAFF]*[_a-z\u3400-\u9FFF\uF900-\uFAFF][_a-z0-9\-\u3400-\u9FFF\uF900-\uFAFF]*)/i, 'class');
    if (tok) {
      this.tokens.push(tok);
      this.incrementColumn(tok.val.length);
      this.tokEnd(tok);
    ...
    if (/^\.[_a-z0-9\-\u3400-\u9FFF\uF900-\uFAFF]+/i.test(this.input)) {
      this.error('INVALID_CLASS_NAME', 'Class names must contain at least one letter or cjkunicode or underscore.');
    }
    if (/^\./.test(this.input)) {
      this.error('INVALID_CLASS_NAME', '"' + /.[^ \t\(\#\.\:]*/.exec(this.input.substr(1))[0] + '" is not a valid class name.  Class names can only contain cjkunicode "_", "-", a-z and 0-9, and must contain at least one of "_", or a-z');
    }
  },
